...
const services = [
  {
    id: 1,
    icon: <FaMotorcycle className="w-10 h-10 text-amber-400" />,
  ...
  {
    id: 2,
    icon: <FaBicycle className="w-10 h-10 text-amber-300 opacity-60" />,
  ...
  {
    id: 3,
    icon: <FaUtensils className="w-10 h-10 text-amber-300 opacity-60" />,
    title: 'Food Delivery',
  ...
  {
    id: 4,
    icon: <FaUserFriends className="w-10 h-10 text-amber-300 opacity-60" />,
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service) => (
            <div
              key={service.id}
              className={`rounded-2xl p-8 flex flex-col items-center bg-neutral-900 shadow-xl border border-amber-400/10 transition-all duration-300 group cursor-pointer ${service.status === 'coming' ? 'opacity-60 grayscale pointer-events-none' : 'hover:scale-105 hover:shadow-amber-400/30 hover:border-amber-400/40'}`}
            >
              <div className="mb-4 group-hover:scale-110 transition-transform duration-300">{service.icon}</div>
              <h3 className="text-xl font-bold text-amber-400 mb-2 text-center group-hover:text-amber-300 transition-colors duration-200">{service.title}</h3>
              <p className="text-zinc-200 text-center text-base">{service.description}</p>
              {service.status === 'coming' && (
                <span className="mt-4 px-4 py-1 rounded-full bg-amber-400/20 text-amber-300 text-xs font-semibold">Coming Soon</span>
              )}
            </div>
          ))}
        </div>
